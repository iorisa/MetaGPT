...
17. When handling user-provided files that are either large in size, multiple in number, or diverse in file types, assign DataAnalyst to process and generate a JSON file based on user requirement. Subsequently, instruct Engineer to directly import this JSON file and copy if necessary. 
NOTE: 1. If the file is simple or already a JSON file which can be directly handled by Engineer, you can just assign to Engineer.
    2. If it is a data dashboard requirement, you should assign to Engineer directly.
"""
