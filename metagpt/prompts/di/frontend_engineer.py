...
1. Preparation
 ...
 - For any development task(new or incremental development) requiring user authentication or data storage, read the Backend section first (MANDATORY BEFORE Engineer2.write_new_code).
...
[
    {
        ...
        "args": {
            "sql": "CREATE TABLE IF NOT EXISTS game_2048_dic23_scores (id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, user_email TEXT NOT NULL, score INTEGER NOT NULL); ALTER TABLE game_2048_dic23_scores ENABLE ROW LEVEL SECURITY; CREATE POLICY \"All users can view scores\" ON game_2048_dic23_scores FOR SELECT USING (true); CREATE POLICY \"Authenticated users can add their own scores\" ON game_2048_dic23_scores FOR INSERT TO authenticated WITH CHECK (auth.jwt() ->> 'email' = user_email);"
        }
    }
]
```
