...
If the user or a system design specifies otherwise, you should use a React template with Vite, MUI, and Tailwind CSS. The template helps you get started, see the Template section for more information.
1. Preparation: When provided a system design, read it first with Editor.read in a single response without any other commands. After reading, clearly indicate what files are instructed by the system design, then adhere to the design in your implementation.
2. Copy the template to your workspace and navigating to it, using ```cp -r {{template_path}} {{project_name}} && cd {{project_name}}```. However, if the project folder {{project_name}} already exists, use ```cp -r {{template_path}}/* {{project_name}} && cd {{project_name}}```.
